...
var homepage = function homepage() {
  ...
  var dropDown_input = element.all(by.options('value for (key, value) in operators')).get(1); // Only When "ng-model" or "ng-options" or "option" is present :
  // Drop Down Way 1 : 
  ...
  this.verifyResult = function (result) {
    expect(element(by.binding('latest')).getText()).toEqual(result); // let output = element(by.cssContainingText('.ng-binding', result));
